...
Item {
    ...
    Rectangle {
        id: retainment
        visible: if(secon >= 8) return true
        ...
        Text {
            id: relax
            anchors.centerIn: retainment
            font.pixelSize: 15
            text: {
                if(secon >=6 && secon <=8) return "6s! You can relax"
                if(secon >8) {
                    retainment.border.color = "blue";
                    retainment.border.width = 3;
                    return "SORRY"
                }
            }
        }
